...
import com.team.bromall.mapper.OmsOrderOperateHistoryMapper;
...
public class OrderServiceImpl implements OrderService {
    ...
    @Override
    public List<OmsOrder> list(OrderQueryParam queryParam, Integer pageSize, Integer pageNum) {
    ...
    public OrderDetail detail(Long id) {
        OrderDetail orderDetail = orderMapper.getDetail(id);
        // 处理商品属性便于前端显示
        int index = 0;
        for (OmsOrderItem orderItem: orderDetail.orderItemList) {
            String attr = orderItem.getProductAttr();
            attr = attr.replace("\"key\":", "").replace("\"value\":", "")
                    .replace("\",\"",":").replace("\"},{\"","; ")
                    .replace("[{\"","").replace("\"}]","");
            orderDetail.orderItemList.get(index).setProductAttr(attr);
            index++;
        }
        return orderDetail;
    }
